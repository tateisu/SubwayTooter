...
object ActionUtils {
	...
	internal interface FindAccountCallback {
		// return account information
		// if failed, account is null.
		fun onFindAccount(who : TootAccount?)
	}
	
	// ユーザ名からアカウントIDを取得する
	internal fun findAccountByName(
		activity : ActMain, access_info : SavedAccount, host : String, user : String, callback : FindAccountCallback
	) {
	...
	internal fun saveUserRelation(
		access_info : SavedAccount, src : TootRelationShip?
	) : UserRelation? {
	...
	private const val CROSS_ACCOUNT_SAME_INSTANCE = 2
	internal const val CROSS_ACCOUNT_REMOTE_INSTANCE = 3
